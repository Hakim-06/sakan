...
const I = {
  ...
  pushpin: (p) => <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 3h6l-1 6 3 3H7l3-3-1-6z"/><line x1="12" y1="12" x2="12" y2="21"/></svg>,
...
export default function Feed() {
  ...
  // ── HANDLERS ──
  const unreadConversations = conversations.filter((c) => Number(c.unread || 0) > 0).length;
  const totalUnread = unreadConversations || (unreadCount > 0 ? 1 : 0);
  ...
  return (
    <div style={{ background:bg, color:text, minHeight:'100vh', width:'100%', display:'flex', flexDirection:'column', fontFamily:"'Inter',-apple-system,sans-serif", position:'relative', overflowX:'hidden', transition:'background 0.4s,color 0.3s', paddingBottom:shouldShowMobileNav?mobileBottomInset:'0' }}>
      ...
      <div ref={aiPanelRef} style={{ position:'fixed', right:isMobile?'10px':'20px', bottom:isMobile?'calc(86px + env(safe-area-inset-bottom, 0px))':'20px', zIndex:1200 }}>
        {isAiOpen && (
          <div style={{ width:isAiCompactMobile?'calc(100vw - 24px)':'325px', maxWidth:isAiCompactMobile?'calc(100vw - 24px)':'calc(100vw - 28px)', height:isAiCompactMobile?'62dvh':'480px', maxHeight:isAiCompactMobile?'calc(100dvh - 130px - env(safe-area-inset-top, 0px))':'none', background:surface, border:`1px solid ${borderStrong}`, borderRadius:isAiCompactMobile?'20px':'18px', boxShadow:`0 30px 70px rgba(0,0,0,${darkMode?'0.5':'0.18'})`, overflow:'hidden', display:'flex', flexDirection:'column', animation:'modalIn 0.25s cubic-bezier(0.16,1,0.3,1)', position:'absolute', right:0, bottom:isAiCompactMobile?'62px':'64px' }}>
              ...
              <div style={{ flex:1, overflowY:'auto' }}>
                ...
                {filteredConversationsList.map(conv=>(
                  <div key={conv.id} onClick={()=>setActiveConvId(conv.userId)}
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'3px' }}>
                        <span style={{ fontWeight:conv.unread>0?'800':'700', color:text, fontSize:'0.86rem', display:'flex', alignItems:'center', gap:'6px' }}>
                          {conv.name}
                          {pinnedConversations.includes(String(conv.userId)) && <I.pushpin width="11" height="11" style={{ color:'#ea580c' }} />}
                        </span>
                    ...
                    >
                      <I.pushpin width="13" height="13" />
                    </button>
                    {conv.unread>0 && <span style={{ background:'#ea580c', color:'white', fontSize:'10px', fontWeight:'800', width:'18px', height:'18px', display:'flex', justifyContent:'center', alignItems:'center', borderRadius:'50%', flexShrink:0, animation:'popIn 0.3s cubic-bezier(0.16,1,0.3,1)' }}>1</span>}
                  </div>
                ))}
              </div>
            ) : activeChat ? (
